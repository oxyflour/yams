...
THREE.W3Character = function(geometries) {
	...
	for (var i = 0, geo; geo = geometries[i]; i ++) {
		var path = geo.extra.TexturePath,
			mat = materialCache[path]
		if (!mat && path) {
			var texture = THREE.ImageUtils.loadTexture(path, THREE.UVMapping)
			texture.flipY = false
			mat = materialCache[path] = new THREE.MeshPhongMaterial({
				map:texture,
				alphaTest:0.1,
				side:geo.extra.TwoSided ? THREE.DoubleSide : THREE.FrontSide
			})
		}
		if (!mat)
			mat = materialCache.basic
		mat.skinning = true
	...
	this.playAnimation = function(name, speed, time) {
		this.root.children.forEach(function(mesh) {
			...
			if (anim !== mesh.animations[name]) {
				...
				if (anim = mesh.animPlaying = mesh.animations[name]) {
					anim.weightDelta =  1/0.3
					anim.play(time >= 0 ? time : anim.currentTime, anim.weight)
				}
			}
